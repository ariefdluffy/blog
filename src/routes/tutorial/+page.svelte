...
<SEO
    title="Tutorial"
    description="Kumpulan tutorial teknis step-by-step berdasarkan pengalaman nyata"
/>
...
<section class="mb-10">
    ...
    >
        ...
        >
            Panduan teknis step-by-step berdasarkan pengalaman nyata setup dan
            maintenance server.
        </p>
    ...
    <section class="mb-8">
        <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-xs text-text-muted dark:text-dark-text-muted"
                >{data.top5.length} dari {data.totalCount}</span
            >
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {#each data.top5 as t, i}
                <a
                    href={t.path}
                    ...
                    <div class="p-4">
                        <div class="flex items-start gap-3">
                            ...
                            <div class="min-w-0 flex-1">
                                <h3
                                    class="text-sm font-semibold group-hover:text-primary transition-colors line-clamp-2"
                                >
                                    {t.title}
                                </h3>
                                <p
                                    class="text-[11px] text-text-muted dark:text-dark-text-muted mt-1.5 font-mono truncate"
                                >
                                    {t.path}
                                </p>
                            </div>
...
{#if data.totalCount > 5}
    <div class="text-center mb-8">
        ...
        >
            <BookOpen size={14} />
            Lihat {data.totalCount - 5} tutorial lainnya
        </button>
    </div>
{/if}

<!-- All Categories (hidden by default if >5) -->
<div
    id="all-categories"
    class="space-y-8 {data.totalCount > 5 ? 'hidden' : ''}"
>
    {#if data.categories.length > 0}
            ...
            <section>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {#each cat.tutorials as t, i}
                        <a
                            href={t.path}
                            class="group block rounded-lg border border-border dark:border-dark-border border-l-2 {getColor(
                                ci,
                            )} hover:border-primary/40 hover:bg-bg-alt dark:hover:bg-dark-bg-alt transition-all overflow-hidden"
                        >
                            <div class="p-4">
                                <div class="flex items-start gap-3">
                                    <span
    ...
    {:else}
        <div
            class="text-center py-16 text-sm text-text-muted dark:text-dark-text-muted italic"
        >
            Belum ada tutorial
        </div>
    {/if}
...
<!-- Footer -->
<section
    class="mt-12 pt-6 border-t border-border dark:border-dark-border text-center"
>
    <p class="text-xs text-text-muted dark:text-dark-text-muted italic">
        Semua tutorial ditulis berdasarkan pengalaman nyata. Langsung bisa
        dipraktekkan.
    </p>
</section>
